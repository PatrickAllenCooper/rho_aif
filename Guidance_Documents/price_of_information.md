...
Shadow-price staircases remain supporting (report brackets + SEs; local non-monotonicity persists on Bandit/Tileworld) [Corrected 2026-08-28: this line previously named Tiger/Diagnosis/Tileworld. results_price_usage_curves.csv shows Tiger and Diagnosis exactly nondecreasing at every sampled grid point; the non-monotone staircases are Bandit and Tileworld.].
...
## 11. Stage G2 — distractor robustness (reward-irrelevant sensing)

Reviewer 8Evk's question on the earlier NeurIPS submission: how poorly do these methods work in settings with uncertainty over state components that happen to be unimportant? No existing benchmark isolates this, since every test in every prior environment is informative about the reward-bearing state. `rho_aif/environments/distractor_diagnosis.py` adds `DistractorDiagnosisEnv`: a 4-condition Diagnosis joined with an independent binary nuisance factor (8 joint states), with a third test that is informative only about the nuisance bit and a commit reward that depends only on the condition. Every observation model in the environment factors as a function of exactly one component (condition tests never touch the nuisance bit and vice versa), so a belief that starts factorized as `P(condition) x Q(nuisance)` — true at reset, since the two components are sampled independently — stays factorized under every update; this is proved and unit-tested directly (`tests/test_distractor_diagnosis.py::TestDistractorCarriesNoTaskInformation`), not just asserted.

The environment reuses the standard `get_obs_models` / `make_env_config` / observe-then-commit agent interface unmodified, so `PlanningAgent`, `PlanningInfoGainAgent`, `EFEAgent`, and `IDSAgent` (Russo & Van Roy 2014) all run against it with no special-casing, directly satisfying the plan's conditional inclusion of IDS as a reward-aware contrast.

**Sweep** (`experiments/run_distractor_diagnosis.py`, 5 seeds x 100 episodes, log grid `w in [0,100]`, `results/results_distractor_diagnosis.csv`, figure `figures/distractor_composition.{png,pdf}`):

| Agent | Usage | Task tests | Distractor tests | Distractor fraction |
|---|---|---|---|---|
| Planning+IG, `w<=3.16` | 5.7–9.7 | same | 0.00 | 0.000 |
| Planning+IG, `w=10` | 12.37 | 9.42 | 2.95 | 0.242 +- 0.005 |
| Planning+IG, `w=31.6..100` | ~20.1 | ~13.4 | ~6.6 | 0.331 +- 0.005 |
| EFE (`w=1`) | 9.77 | 9.77 | 0.00 | 0.000 |
| IDS | 21.81 | 14.96 | 6.86 | 0.311 +- 0.005 |

**Finding 1 (expected):** ordinary information gain is reward-blind. Planning+IG spends a growing, saturating fraction of its sensing budget on the distractor as `w` increases past the point where task uncertainty is already well resolved (roughly a third of usage at `w>=31.6`); a sensing budget caps the total spend but does not redirect it away from reward-irrelevant tests. `EFE (w=1)` stays at the untuned default and never touches the distractor on this instance, but that is a consequence of `w=1` being small relative to the onset weight here (`w~10`), not evidence that EFE is immune to the failure mode in general — the same PI-1 scale-equivariance result (Section 9) that makes `w=1` not scale-invariant applies here too: rescaling rewards down, or the distractor's own informativeness up, would move the onset below `w=1`.

**Finding 2 (unexpected, verified at the code level):** IDS is not a clean reward-aware immune baseline in this environment, contrary to the a priori expectation that minimizing regret-per-information-about-the-optimal-action should ignore a test that cannot change which condition is likely. Instrumenting `IDSAgent._obs_regret_and_info` directly confirms the mechanism: for the distractor action, `info_astar` (information about the optimal-commit distribution) is exactly `0.0` — consistent with the factorization proof above, since the condition marginal, and therefore the optimal-commit distribution, is unchanged by a distractor observation — but the implementation's fallback (`info = info_astar if info_astar > eps else info_state`) substitutes the raw state-entropy reduction `info_state`, which the distractor test does provide (it genuinely resolves the nuisance bit). This fallback exists to keep IDS from stalling when `info_astar~0` for every action at a genuinely uncertain belief; it was not designed with a reward-irrelevant nuisance factor in mind, and here it lets a reward-irrelevant test look valuable by the same route Planning+IG's raw IG term does. IDS's fraction (0.311) and reward (-11.93 +- 0.31) end up close to Planning+IG at `w>=31.6`, not close to EFE or Planning. We report this as a genuine, code-verified finding about the specific `russo2014`-style IDS adaptation implemented here, not a general claim about information-directed sampling.

**Limitations paragraph (for the draft):** on an environment with reward-irrelevant sensing options, an unconstrained information-gain bonus (Planning+IG at moderate-to-large `w`, and this repository's IDS adaptation once its `info_astar` fallback triggers) will spend a nonzero and non-decreasing share of its sensing budget on those options; a sensing budget caps the total but does not redirect it toward reward-relevant tests. Practitioners deploying EFE-as-`rho` or a tuned information bonus in a domain with known-irrelevant sensing channels should either exclude those channels from the observation menu or use a genuinely reward-conditioned information measure (e.g., an information-ratio criterion without a raw-entropy fallback, or expected value of information restricted to the optimal-action posterior) rather than assume default reward-awareness protects them.

Citation check: Russo & Van Roy 2014 (`russo2014`, already in the verified bibliography) is the only reference in this section; no new citations were introduced.
