...
\begin{abstract}
We bridge $\rho$-POMDPs and active inference by showing that Expected Free Energy (EFE) is equivalent to augmenting reward with information gain at a canonical weight $w{=}1$ derived from the variational bound. We evaluate this canonical choice against five baselines---including same-horizon reward-only planning and Planning+IG at per-environment tuned weights---across six discrete POMDP environments. On multi-observation-action environments where the agent must choose \emph{which} information to gather, $w{=}1$ significantly outperforms same-horizon planning ($+9.4$ pp on diagnosis, $+15.5$ pp on a structured bandit; $p < 0.05$) and achieves substantially better reward than tuned alternatives that over-explore. On a spatial Tileworld ($8{\times}8$, $|S|{=}64$), the EFE-based agent achieves $75.5\%$ success where reward-only planning collapses to $2.0\%$. A Pareto analysis shows $w{=}1$ sits near the success-reward knee on all environments without per-environment search, while tuned weights ($w^*{=}20$--$100$) sacrifice reward for accuracy. These findings identify EFE as a principled, automatically-calibrated $\rho$ function for structured uncertainty.
\end{abstract}
...
\begin{enumerate}
    ...
    \item Characterization of the regimes where EFE-as-$\rho$ provides genuine advantage, demonstrated on a spatial Tileworld where VFE's advantage grows with state space size ($75.5\%$ vs.\ $2.0\%$ at $|S|{=}64$).
\end{enumerate}
...
All environments follow an observe-then-commit structure as OpenAI Gymnasium environments: the agent selects among observation actions (gathering noisy information) and commit actions (terminating the episode). Experiments use 1{,}000 episodes (500 for scaling) with a fixed random seed. Statistical comparisons use $t$-tests with Holm-Bonferroni correction; bootstrap CIs and Cohen's $d$ are in Appendix~\ref{app:stats}. Full specifications are in Appendix~\ref{app:envs}.
...
Two additional environments---a two-state testbed (Appendix~\ref{app:testbed}) and navigation (Appendix~\ref{app:navigation})---delimit VFE's applicability on mild-penalty and small-state-space settings.
...
\begin{table}[ht]
\centering
\caption{Results across three core environments (1{,}000 episodes each). $w^*$: per-environment tuned weight. Standard errors and full agent set in Appendix~\ref{app:full_tables}.}
\label{tab:main}
\begin{small}
\begin{tabular}{llccc}
\toprule
...
\midrule
Tiger & Myopic & $1.00$ & 85.0\% & $-7.50$ \\
\small{($H{=}6$, $w^*{=}20$)} & Planning & $4.28$ & 99.5\% & $+5.17$ \\
& Planning+IG & $4.18$ & 99.2\% & $+4.94$ \\
& \textbf{VFE} & $\mathbf{4.17}$ & $\mathbf{99.9\%}$ & $\mathbf{+5.72}$ \\
\midrule
Diagnosis & Myopic & $2.00$ & 63.1\% & $-14.14$ \\
\small{($H{=}3$, $w^*{=}100$)} & Planning & $5.80$ & 87.6\% & $-3.24$ \\
& Planning+IG & $13.13$ & 98.5\% & $-4.03$ \\
& \textbf{VFE} & $\mathbf{9.73}$ & $\mathbf{97.0\%}$ & $\mathbf{-1.53}$ \\
\midrule
Bandit & Myopic & $1.99$ & 62.9\% & $+5.66$ \\
\small{($H{=}2$, $w^*{=}100$)} & Planning & $3.17$ & 70.0\% & $+5.72$ \\
& Planning+IG & $12.43$ & 99.9\% & $+3.78$ \\
& \textbf{VFE} & $\mathbf{5.24}$ & $\mathbf{85.5\%}$ & $\mathbf{+6.08}$ \\
\bottomrule
\end{tabular}
\end{small}
\end{table}

On Tiger (single observation action), all multi-step agents achieve ${\sim}99\%$ success; VFE matches tuned alternatives without weight selection. The Epistemic-only ablation commits at chance ($50.9\%$ on Tiger, $24.5\%$ on Bandit; Appendix~\ref{app:full_tables}), proving the pragmatic term is essential.

On the multi-observation-action environments, VFE's advantage emerges clearly. On Diagnosis, VFE outperforms same-horizon Planning ($97.0\%$ vs.\ $87.6\%$, $p < 0.001$) and achieves substantially better reward than Planning+IG ($-1.53$ vs.\ $-4.03$, $p < 0.001$): the additive weight ($w{=}100$) drives 13.13 tests where VFE's self-limiting objective stops at 9.73. On Bandit, VFE achieves the highest reward ($+6.08$, $p < 0.001$ vs.\ all) while Planning+IG over-explores (12.43 inspections, $+3.78$ reward). The pattern is consistent: EFE's joint pragmatic--epistemic objective knows when to stop exploring, while additive IG bonuses with tuned weights do not. Additional environments---a two-state testbed where VFE over-explores under mild penalties, and navigation where VFE is counterproductive on a $3{\times}3$ grid---delimit VFE's applicability and are reported in Appendices~\ref{app:testbed} and \ref{app:navigation}.

\subsection{Pareto analysis: the canonical $w{=}1$}
\label{sec:pareto}

By Proposition~\ref{prop:equivalence}, VFE is exactly Planning+IG with $w{=}1$. We sweep $w$ from $0.01$ to $200$ on all environments (Figure~\ref{fig:pareto}).

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_pareto.pdf}
\caption{Pareto analysis: success rate vs.\ mean reward as $w$ varies from 0.01 to 200. Diamond: $w{=}1$ (canonical EFE weight). Star: VFE agent. On all environments, $w{=}1$ sits at or near the Pareto knee---the inflection where further weight increases buy marginal accuracy at substantial reward cost.}
\label{fig:pareto}
\end{figure}

On every environment, $w{=}1$ sits near the reward-maximizing weight $w^*_{\text{ret}}$, while the success-maximizing weight $w^*_{\text{succ}}$ lies at $20$--$200$. The canonical $w{=}1$ achieves high reward with good (but not maximal) accuracy without per-environment search---the contribution of EFE is deriving this near-optimal weight from the variational bound rather than a grid search whose optimum changes by orders of magnitude across tasks.

\subsection{Tileworld: spatial epistemic foraging}
\label{sec:tileworld}

The Tileworld projects the Diagnosis partition structure onto an $N{\times}N$ grid ($|S|{=}N^2$), producing spatially interpretable belief evolution (Figure~\ref{fig:tw_comparison}; step-by-step belief strips in Appendix~\ref{app:tw_belief}).
...
VFE achieves the highest reward ($-20.73$), scanning 14.89 times---less than half of Planning+IG's 33.49 scans, which erode reward despite reaching 98.6\% success. The same over-exploration pattern from Diagnosis and Bandit recurs at this larger scale. Figure~\ref{fig:tw_comparison} visualizes the mechanism: VFE concentrates belief efficiently via partition-based narrowing and commits once confident, while Planning lacks scan-selection guidance and InfoGain-Tuned continues scanning past the point of diminishing returns.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_tileworld_comparison.pdf}
\caption{Agent comparison on the same $6{\times}6$ Tileworld episode. VFE (top) commits correctly in 16 scans; Planning (middle) commits incorrectly; InfoGain-Tuned (bottom) over-scans to 27 steps. Red circle: committed cell; green star: target.}
\label{fig:tw_comparison}
\end{figure}

\paragraph{Spatial scaling.} We scale the grid from $4{\times}4$ to $8{\times}8$ (Figure~\ref{fig:tw_scaling}). At $8{\times}8$ ($|S|{=}64$), Planning collapses to $2.0\%$ success while VFE maintains $75.5\%$. With 64 states and only 6 scans, the reward signal alone is too diffuse to justify scanning at $H{=}2$; EFE's epistemic term ensures each scan is valued for its information content, driving systematic region narrowing even when immediate reward is uninformative.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_tileworld_scaling.pdf}
\caption{Tileworld scaling ($H{=}2$, 200 episodes). VFE maintains high success as state space grows; Planning collapses at $8{\times}8$ ($|S|{=}64$).}
\label{fig:tw_scaling}
\end{figure}

\subsection{Summary}

The results reveal a consistent pattern: (i)~VFE matches Planning+IG at $w{=}1$ on all environments, confirming Proposition~\ref{prop:equivalence}; (ii)~$w{=}1$ sits at the Pareto knee without per-environment search; (iii)~on multi-observation-action environments (Diagnosis, Bandit, Tileworld), VFE significantly outperforms same-horizon planning ($p < 0.05$); (iv)~VFE's advantage grows with state space size ($75.5\%$ vs.\ $2.0\%$ at $8{\times}8$); and (v)~the Epistemic-only ablation collapses on all environments, confirming that the pragmatic term makes epistemic foraging instrumentally rational.

\section{Discussion}

\paragraph{When does EFE-as-$\rho$ help?}
VFE's advantage requires two conditions: (1)~multiple observation actions with differential informativeness, and (2)~sufficient planning horizon for recursive EFE to propagate epistemic value. When (1) fails (Tiger), reward-only planning matches VFE. The Tileworld adds a third axis: as the state space grows, the reward signal becomes too diffuse to guide scan selection, and VFE's advantage \emph{increases} (Figure~\ref{fig:tw_scaling}). Within-episode dynamics (Appendix~\ref{app:foraging}) show the mechanism: VFE concentrates belief via partition-based narrowing and commits once the value of committing exceeds the value of observing---a crossover that emerges automatically from $w{=}1$ without tuning (Figure~\ref{fig:traj}, Appendix~\ref{app:efe_trajectory}).

\paragraph{Why $w{=}1$ works.}
EFE does not eliminate the exploration--exploitation weight; it \emph{derives} one from the variational bound. The Pareto analysis (Figure~\ref{fig:pareto}) shows this weight sits near the reward-maximizing $w^*_{\text{ret}}$ on every environment, while $w^*_{\text{succ}}$ lies at $20$--$200$. Increasing $w$ beyond 1 buys marginal accuracy at substantial reward cost. Moreover, adding planning depth to a weighted IG bonus \emph{amplifies} the weight's effect ($w{=}100$ on Bandit: Planning+IG takes 12.43 inspections vs.\ InfoGain's 10.95), making the tuning problem harder with depth. EFE avoids this because both terms share a common scale.

\paragraph{Limitations and future work.}
All environments use observe-then-commit structure; extending to interleaved observe-act settings is important future work. The recursive cost $\mathcal{O}(K \cdot |\mathcal{O}|^H)$ limits practical horizons to $H{=}2$--$3$ with $K \geq 3$; replacing exact tree search with MCTS using EFE as a leaf heuristic \citep{benchetrit2025, maisto2025} is a natural extension. The VFE agent assumes a known generative model; extending to model learning (where AIF and BAMDPs converge) is a further direction. On small spatial domains ($3{\times}3$), VFE is counterproductive (Appendix~\ref{app:navigation}), suggesting a minimum scale around $|S| \geq 16$.
...
\begin{table}[h]
\centering
\caption{Information-seeking testbed (1{,}000 episodes, $H{=}4$). Symmetric rewards $+1 / {-}1$. The tuning procedure selects $w^*{=}1$ for Planning+IG, making it identical to VFE by Proposition~\ref{prop:equivalence}.}
\begin{small}
\begin{tabular}{lccc}
\toprule
...
Under mild reward asymmetry ($+1 / {-}1$), VFE gathers more information than is instrumentally optimal ($+0.36$ vs.\ Planning's $+0.46$, $p < 0.001$) because uncertainty reduction has intrinsic value under EFE. Tuned Info Gain ($w{=}50$) over-explores catastrophically: 99.9\% success but negative reward. This delimits VFE's applicability: under very mild penalties, EFE's constitutive epistemic drive can be counterproductive.
...
To test whether VFE's Tiger parity holds across reward scales, we sweep the penalty magnitude from $|R^-|{=}1$ to $500$. VFE naturally adapts its exploration depth, tracking the reward-optimal strategy without reconfiguration. InfoGain-Tuned collapses at low penalties where its weight ($w{=}20$, tuned at $|R^-|{=}100$) drives excessive listening.
...
\caption{Observation action scaling on Diagnosis ($N{=}8$, $H{=}3$, 500 episodes). VFE's advantage over Planning grows with $K$, confirming that EFE is most valuable when the agent must choose among differentially informative observations.}
\label{fig:obs_scaling}
\end{figure}

Holding $N{=}8$ fixed on Diagnosis and varying $K$ from 1 to 3 isolates the ``which information'' hypothesis. At $K{=}1$, all agents face the same single test and perform similarly. As $K$ increases, VFE's advantage over Planning grows because the information gain term guides test selection.
