...
We compare six agents (Table~\ref{tab:agents}), all sharing the same belief-update machinery and differing only in objective function and planning depth. We label the EFE-minimizing agent ``VFE'' after the variational free energy framework from which EFE is derived, distinguishing the agent name from the objective $\mathcal{G}$. The Planning and Planning+IG baselines use the same recursive tree search as VFE, isolating the effect of the $\rho$ function from planning depth.
...
\midrule
\multirow{4}{*}{\shortstack[l]{Tiger\\[-1pt]{\scriptsize $H{=}6,\;w^*{=}20$}}} & Myopic & $1.00$ & 85.0\% & $-7.50$ \\
& Planning & $4.28$ & 99.5\% & $+5.17$ \\
& Planning+IG & $4.18$ & 99.2\% & $+4.94$ \\
& \textbf{VFE} & $\mathbf{4.17}$ & $\mathbf{99.9\%}$ & $\mathbf{+5.72}$ \\
\midrule
\multirow{4}{*}{\shortstack[l]{Diagnosis\\[-1pt]{\scriptsize $H{=}3,\;w^*{=}100$}}} & Myopic & $2.00$ & 63.1\% & $-14.14$ \\
& Planning & $5.80$ & 87.6\% & $-3.24$ \\
& Planning+IG & $13.13$ & 98.5\% & $-4.03$ \\
& \textbf{VFE} & $\mathbf{9.73}$ & $\mathbf{97.0\%}$ & $\mathbf{-1.53}$ \\
\midrule
\multirow{4}{*}{\shortstack[l]{Bandit\\[-1pt]{\scriptsize $H{=}2,\;w^*{=}100$}}} & Myopic & $1.99$ & 62.9\% & $+5.66$ \\
& Planning & $3.17$ & 70.0\% & $+5.72$ \\
...
\caption{Agent comparison on the same $6{\times}6$ Tileworld episode. VFE (top) commits correctly in 16 scans; Planning (middle) commits incorrectly; Info Gain (bottom) over-scans to 27 steps. Red circle: committed cell; green star: target.}
...
\caption{Reward asymmetry sweep on Tiger ($H{=}6$, 500 episodes per point). VFE and Planning adapt smoothly across three orders of magnitude in penalty; Info Gain's fixed weight produces inconsistent performance.}
\label{fig:sweep}
\end{figure}

To test whether VFE's Tiger parity holds across reward scales, we sweep the penalty magnitude from $|R^-|{=}1$ to $500$. VFE naturally adapts its exploration depth, tracking the reward-optimal strategy without reconfiguration. Info Gain collapses at low penalties where its weight ($w{=}20$, tuned at $|R^-|{=}100$) drives excessive listening.
...
The aggregate statistics in Table~\ref{tab:main} show \emph{what} VFE achieves; we now visualize \emph{how} it achieves it by examining within-episode dynamics on extended Diagnosis instances ($N{=}8$, $K{=}3$, accuracy 0.75) that produce episodes with 8--15+ observation steps.

\paragraph{Belief evolution.}
Figure~\ref{fig:belief_heatmap} shows belief-state evolution within representative episodes for VFE, Planning, and Info Gain. VFE exhibits systematic partition-based narrowing: early tests eliminate large groups of states, and later tests refine among remaining candidates. Planning (without epistemic value) makes observations but lacks guidance on \emph{which} test to run, producing less structured belief concentration. Info Gain continues testing well past the point where the agent has high confidence.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig_belief_heatmap.pdf}
\caption{Belief evolution within representative Diagnosis episodes ($N{=}8$, $K{=}3$, accuracy 0.75). Darker = higher probability. Green: true state. Dashed: commit point. VFE concentrates belief efficiently; Info Gain over-explores.}
