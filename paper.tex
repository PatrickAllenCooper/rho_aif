...
On Tiger, all multi-step agents (Planning, Planning+IG, VFE) achieve ${\sim}99\%$ success. VFE achieves the highest reward ($+5.72$, $p = 0.024$ vs.\ Planning+IG), but the differences are modest. The critical finding is the Epistemic-only ablation: without the pragmatic (reward) term, the agent commits immediately at chance (50.9\%), proving that the pragmatic term is essential and the advantage of VFE is not reducible to information gain alone. On simple observe-then-commit environments, VFE achieves parity with tuned alternatives without per-environment weight selection.
...
The Planning+IG baseline is the strongest alternative: same recursive tree search as VFE, same epistemic bonus concept, same horizon. Yet VFE consistently achieves better reward. On the Bandit, VFE earns $+6.08$ vs.\ Planning+IG's $+3.78$ ($p < 0.001$); on Diagnosis, $-1.53$ vs.\ $-4.03$ ($p < 0.001$). The mechanism: Planning+IG's additive weight, even when tuned, lacks a natural stopping criterion---it over-explores because the weight that achieves high \emph{accuracy} also drives excessive observation. EFE's joint pragmatic-epistemic objective is self-limiting: as beliefs sharpen, $I(b)$ decays toward zero and the pragmatic commit value dominates, producing a natural transition from exploration to exploitation without weight tuning.
