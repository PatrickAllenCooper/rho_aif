...
To characterize the conditions under which $w{=}1$ is near-optimal, consider a two-state observe-then-commit POMDP with reward asymmetry ratio $\alpha = |R^-|/|R^+|$ and observation informativeness $\eta = I_{\max} / c$ (maximum information gain divided by observation cost). For $H{=}1$, the reward-maximizing weight satisfies $w^*_{\text{ret}} \to 1$ as $\alpha\,\eta \to \infty$. Intuitively, when penalties are severe relative to rewards ($\alpha \gg 1$) and observations are informative relative to their cost ($\eta \gg 1$), the reward-optimal strategy is to gather exactly as much information as EFE prescribes. When $\alpha \approx 1$ (the Testbed regime), the observation cost dominates the misidentification penalty, and $w^*_{\text{ret}} < 1$ --- consistent with our finding that EFE over-explores on the Testbed. Our environments span $\alpha$ from 1 (Testbed) to 10 (Tiger), with $w{=}1$ near-optimal for all $\alpha \geq 5$ (Appendix~\ref{app:testbed}).
