...
- **Blocking manual steps (Patrick only, credential-gated)**:
  1. `twine upload` to TestPyPI then PyPI (`rho-aif` name verified unclaimed). The `pip install rho-aif` claim (README, JAIR checklist) is false until this lands.
  2. Push commits and tags to `origin` (local `main` is well ahead of `origin/main`; `v2.0.0` is tagged locally, not yet pushed).
