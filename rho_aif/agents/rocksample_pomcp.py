...
class RockSampleRolloutOnlyAgent:
    """RockSamplePOMCPAgent's rollout policy run standalone, with no search tree.

    Every POMCP row should be reported alongside this agent at the same
    protocol. On RockSample the rollout policy carries most of the score, so
    without the companion row a reader cannot tell how much the search tree
    adds, and the POMCP number can be dismissed in either direction.

    It shares the POMCP agent's belief update and rollout policy exactly, so
    the comparison isolates the tree and nothing else.
    """
    ...
    def __init__(self, env, rollout_policy: str = "approach", seed: Optional[int] = None):
        ...
        self.env = env
        self.rollout_policy = rollout_policy
        self.belief = self._inner.belief
        self.diagnostics_history: List[Dict] = []
